<template>
    <div v-if="isFixedHeader">
        <div class="vue2-table-head-wrapper">
            <table :class="['vue2-table', css.tableClass, css.tableHeaderClass]">
                <thead>
                    <tr>
                        <template v-for="(field, fieldIndex) in tableFields">
                            <template v-if="field.visible">
                                <template v-if="isSpecialField(field.name)">
                                    <th v-if="extractName(field.name) == '__checkbox'" :key="fieldIndex"
                                        :style="{ width: field.width }"
                                        :class="['vue2-table-th-checkbox-' + trackBy, field.titleClass]">
                                        <input type="checkbox" @change="toggleAllCheckboxes(field.name, $event)"
                                            :checked="checkCheckboxesState(field.name)">
                                    </th>

                                    <th v-if="extractName(field.name) == '__component'" :key="fieldIndex"
                                        :style="{ width: field.width }"
                                        :class="['vue2-table-th-component-' + trackBy, field.titleClass, sortClass(field), { 'sortable': isSortable(field) }]"
                                        v-html="renderTitle(field)" @click="orderBy(field, $event)">
                                    </th>

                                    <th v-if="extractName(field.name) == '__slot'" :key="fieldIndex"
                                        :style="{ width: field.width }"
                                        :class="['vue2-table-th-slot-' + extractArgs(field.name), field.titleClass, sortClass(field), { 'sortable': isSortable(field) }]"
                                        v-html="renderTitle(field)" @click="orderBy(field, $event)">
                                    </th>

                                    <th v-if="extractName(field.name) == '__sequence'" :key="fieldIndex"
                                        :style="{ width: field.width }"
                                        :class="['vue2-table-th-sequence', field.titleClass || '']"
                                        v-html="renderTitle(field)">
                                    </th>

                                    <th v-if="notIn(extractName(field.name), ['__sequence', '__checkbox', '__component', '__slot'])"
                                        :key="fieldIndex" :style="{ width: field.width }"
                                        :class="['vue2-table-th-' + field.name, field.titleClass || '']"
                                        v-html="renderTitle(field)">
                                    </th>
                                </template>

                                <template v-else>
                                    <th @click="orderBy(field, $event)" :key="fieldIndex" :id="'_' + field.name"
                                        :style="{ width: field.width }"
                                        :class="['vue2-table-th-' + field.name, field.titleClass, sortClass(field), { 'sortable': isSortable(field) }]"
                                        v-html="renderTitle(field)"></th>
                                </template>
                            </template>
                        </template>

                        <th v-if="scrollVisible" :style="{ width: scrollBarWidth }" class="vue2-table-gutter-col"></th>
                    </tr>
                </thead>
            </table>
        </div>

        <div class="vue2-table-body-wrapper" :style="{ height: tableHeight }">
            <table :class="['vue2-table', css.tableClass, css.tableBodyClass]">
                <colgroup>
                    <template v-for="(field, fieldIndex) in tableFields">
                        <template v-if="field.visible">
                            <col :key="fieldIndex" :id="'_col_' + field.name" 
                                :style="{ width: field.width }"
                                :class="['vue2-table-th-' + field.name, field.titleClass]" 
                            />
                        </template>
                ...
                <tbody v-cloak class="vue2-table-body">
                    <template v-for="(item, itemIndex) in tableData">
                        ...
                        >
                            <template v-for="(field, fieldIndex) in tableFields">
                                <template v-if="field.visible">
                                    <template v-if="isSpecialField(field.name)">
                                        <td v-if="extractName(field.name) == '__sequence'" :key="fieldIndex"
                                            :class="['vue2-table-sequence', field.dataClass]"
                                            v-html="renderSequence(itemIndex)">
                                        </td>

                                        <td v-if="extractName(field.name) == '__handle'" :key="fieldIndex"
                                            :class="['vue2-table-handle', field.dataClass]"
                                            v-html="renderIconTag(['handle-icon', css.handleIcon])">
                                        </td>

                                        <td v-if="extractName(field.name) == '__checkbox'" :key="fieldIndex"
                                            :class="['vue2-table-checkboxes', field.dataClass]">
                                            <input type="checkbox" @change="toggleCheckbox(item, field.name, $event)"
                                                :checked="rowSelected(item, field.name)">
                                        </td>

                                        <td v-if="extractName(field.name) === '__component'" :key="fieldIndex"
                                            :class="['vue2-table-component', field.dataClass]">
                                            <component :is="extractArgs(field.name)" :row-data="item" :row-index="itemIndex"
                                                :row-field="field.sortField"></component>
                                        </td>

                                        <td v-if="extractName(field.name) === '__slot'" :key="fieldIndex"
                                            :class="['vue2-table-slot', field.dataClass]">
                                            <slot :name="extractArgs(field.name)" :row-data="item" :row-index="itemIndex"
                                                :row-field="field.sortField"></slot>
                                        </td>
                                    </template>
                        ...
                        <template v-if="useDetailRow">
                            <transition :name="detailRowTransition" :key="itemIndex">
                                <tr v-if="isVisibleDetailRow(item[trackBy])" :class="[css.detailRowClass]"
                    ...
                    <template v-if="displayEmptyDataRow">
                        <tr>
                            <td :colspan="countVisibleFields" class="vue2-table-empty-result" v-html="noDataTemplate"></td>
                        </tr>
                    </template>
    ...
    <table v-else :class="['vue2-table', css.tableClass]"> <!-- no fixed header - regular table -->
        <thead>
            <tr>
                <template v-for="(field, fieldIndex) in tableFields">
                    <template v-if="field.visible">
                        <template v-if="isSpecialField(field.name)">
                            ...
                            <th v-if="extractName(field.name) == '__slot'" :key="fieldIndex" :style="{ width: field.width }"
                                :class="['vue2-table-th-slot-' + extractArgs(field.name), field.titleClass, sortClass(field), { 'sortable': isSortable(field) }]"
                                v-html="renderTitle(field)" @click="orderBy(field, $event)">
                            </th>
                            ...
                            <th v-if="extractName(field.name) == '__sequence'" :key="fieldIndex"
                                :style="{ width: field.width }"
                                :class="['vue2-table-th-sequence', field.titleClass || '', sortClass(field)]"
                                v-html="renderTitle(field)">
                            </th>

                            <th v-if="notIn(extractName(field.name), ['__sequence', '__checkbox', '__component', '__slot'])"
                                :key="fieldIndex" :style="{ width: field.width }"
                                :class="['vue2-table-th-' + field.name, field.titleClass || '', sortClass(field)]"
                                v-html="renderTitle(field)">
                            </th>
                        </template>
...
export default {
    ...
    mounted() {
        ...
        if (this.isFixedHeader) {
            let elem = this.$el.getElementsByClassName('vue2-table-body-wrapper')[0];
    ...
    methods: {
        ...
        handleScroll(e) { //make sure that the header and the body are aligned when scrolling horizontally on a table that is wider than the viewport
            ...
            if (horizontal != this.lastScrollPosition) { //don't modify header scroll if we are scrolling vertically
                let header = this.$el.getElementsByClassName('vue2-table-head-wrapper')[0]
        ...
        loadSuccess(response) {
            ...
            if (this.tablePagination === null) {
                this.warn('vue2-table: pagination-path "' + this.paginationPath + '" not found. '
        ...
        fixHeader() {
            ...
            let element = this.$el.getElementsByClassName('vue2-table-body-wrapper')[0]

            if (element != null) {
                this.scrollVisible = element.scrollHeight > element.clientHeight
            }
        },
        ...
        sortClass(field) {
            let classString = ''
            let currentPosition = this.currentSortOrderPosition(field)

            if (currentPosition !== false) {
                classString = (this.sortOrder[currentPosition].direction == 'asc') ? this.css.ascendingClass : this.css.descendingClass
            }
        ...
        sortIcon(field) {
            let classString = this.css.sortableIcon
            let currentPosition = this.currentSortOrderPosition(field)

            if (currentPosition !== false) {
                classString = (this.sortOrder[currentPosition].direction == 'asc') ? this.css.ascendingIcon : this.css.descendingIcon
            }
        ...
        toggleCheckbox(dataItem, fieldName, event) {
            ...
            this.$emit('vue2-table:checkbox-toggled', isChecked, dataItem)
        },
        ...
        checkCheckboxesState(fieldName) {
            ...
            let idColumn = this.trackBy
            let selector = 'th.vue2-table-th-checkbox-' + idColumn + ' input[type=checkbox]'
            ...
            let selected = this.tableData.filter(item => {
                return this.selectedTo.indexOf(item[idColumn]) >= 0
            })
        ...
        toggleAllCheckboxes(fieldName, event) {
            ...
            this.$emit('vue2-table:checkbox-toggled-all', isChecked)
        },
...
<style scoped>
.vue2-table th.sortable:hover {
    color: #2185d0;
    cursor: pointer;
}

.vue2-table-body-wrapper {
    position: relative;
    overflow-y: auto;
}

.vue2-table-head-wrapper {
    overflow-x: hidden;
}
...
.vue2-table-pagination {
    background: #f9fafb !important;
}

.vue2-table-pagination-info {
    margin-top: auto;
    margin-bottom: auto;
}

.vue2-table-empty-result {
    text-align: center;
}

.vue2-table-clip-text {
...
.vue2-table-semantic-no-top {
    border-top: none !important;
    margin-top: 0 !important;
}

.vue2-table-fixed-layout {
    table-layout: fixed;
}

.vue2-table-gutter-col {
    padding: 0 !important;
